...
### Update — 2026-06-11 s74: the four-bug report closed — locked-look determinism, walk height, doubled doors, THE DRAWBRIDGE ships

Resumed a killed predecessor session; its in-flight work was audited
against the .s, finished, and landed (port commit 8bf0c78 carried its
gib-family + locked-look + walk-height code; this session added the
exporter work, the assets, the verification harness and these docs).

- **Bug 1 (locked-door camera regression) — FIXED + witnessed**: the
  s53 port read the LIVE chase yaw for the func_001BBBF0 eye; the .s
  reads the GLOBAL D_00810374, and the full writer census (FINDINGS
  s72 §3) shows only script/cutscene ops write it — never the chase.
  Port stand-in: the kickoff through-door snap yaw (deterministic per
  door, FLAGGED vs the engine's per-route last-scripted yaw). New
  EM_CAPTURE_LOCKED=1/2 (two approach orientations, prior camera yaw
  0 vs -2.2) print the identical locked-look — eye (-28.5, 12, -205),
  tgt (-28.5, 10, -192).
- **Bug 2 (camera dives to the feet while walking) — FIXED +
  witnessed**: func_00191390 re-read (FINDINGS s74 §4) — ordinary walk
  is player state 3 = the DEFAULT 6.0/2.0 row (eye +19 / tgt +17);
  the -3.0/1.0 low row is the +0x236 ELEVATED family (states 2/4/0xF)
  the s67 binding had wrongly applied to all movement. New
  EM_CAPTURE_WALK prints eye +19.0 / tgt +17.0 mid-run; capture shows
  level over-the-shoulder framing.
- **Bug 3 (doubled office doors) — FIXED**: export_level.py's region
  lists still carried the RGN_DOOR replays s20 had only filtered in
  export_props' rebake, so later direct re-exports re-baked a CLOSED
  door over the articulated EMDLs. Rows removed at the source (both
  CHUNK06N1_REGIONS and table_driven_regions); 00_level.emdl
  regenerated (6115 v / 4037 t / 108 tex, byte-identical re-runs);
  both office doorways captured single-door mid-open.
- **Bug 4 (the missing DRAWBRIDGE) — SHIPPED (rest pose, flagged)**:
  AREA01 table-A model-table placements decoded (FINDINGS s74 §1):
  records [41]/[42] are the TWO mirrored 105 x 60 deck halves (overlay
  fn 0x8261A0, entries 2/3, yaw-pi, meeting at exactly z = -420 over
  the chasm), plus the y-156.9 suspension panels, the crank piece at
  the s69 crank-examine, and two creature-family fixtures.
  `--drawbridge` now bakes them into `12_placed.emdl` (7 instances,
  1972 v / 1144 t, 31/31 textures from the uploads replay);
  office0_bake_placed generalized (table_off). LOWERED rest pose —
  the raise/lower overlay actor (fn 0x8261A0) and the counter-0x80
  lowering-cutscene script stay the open item; deck walkability
  unprobed (visual only).
- **Predecessor audit verdicts**: gib husk-family split (func_001551B0
  @0x156380 rebind: model byte 6 -> husk 0x22, else 0x29) KEPT —
  builds, suite-verified; locked-look snap yaw KEPT (census verified);
  walk-height row KEPT (.s verified); export_level.py door-row removal
  KEPT (asset regenerated byte-identical to its run).
- **Verified**: full port suite 19/19 PASS (slider/locked under
  EM_SCENE=assets/scene_drawbridge); default capture byte-identical vs
  a clean-HEAD rebuild; captures — locked_v1/v2 (identical framing),
  walk_away_f80 (level framing), door_open_f110 + door2_open_f130

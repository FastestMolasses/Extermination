...
**Pending (phase 4 — needs the toolchain):**

- Build the `mwccps2` toolchain container — `docker/Dockerfile` (Debian +
  wibo + MIPS binutils), built/run with **Apple's `container` CLI**,
  `linux/amd64`. **Blocked:** the user must supply the proprietary Metrowerks
  `mwccps2` compiler (it is never committed; lives in `tools/mwccps2/`).
...
splat, objdiff config) are done; phase 4 (toolchain container + first match)
is pending. See "In progress — Track A" above for detail and the immediate
blocker — the user must supply the proprietary Metrowerks `mwccps2` compiler.

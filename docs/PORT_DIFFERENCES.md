...
| C2 | Run-down decay | **DECODED s78** (func_0017BC40 phase 2): decay 0.03125 u/tick, **CASCADES tier-by-tier** (run→jog→walk→stop) to each demote floor (x2=0.0625 with carried gear `+0x314 & 0x1F`); mode-6 stop-skid anims ids 4 (jog) / 5 (run) | decay translated; the impl pass adds the tier CASCADE (gear x2 omitted — port has no gear-carry state, flagged); stop-skid ids 4/5 still untranslated (needs clips 4/5 re-exported — TODO) | B/V | SI + UT (skid TODO) |
| C3 | Facing turn rate | **DECODED s78** (func_00174AC0 + func_001B12B0, FINDINGS "GROUND LOCOMOTION"): speed×angle-BANDED turn rates (turn-in-place gait2/1/0|3 = 4°/8°/22.5° per frame; moving |delta|≤54° walk/jog/run = 4°/6°/7°; >54° = 6°/9°/10.5°), SNAP within one step | ~~`TURN_SPEED` 12 rad/s shortest-arc seek~~ → the banded table (implement pass in progress) | B | PC→decoded (impl in progress) |
| C4 | Move heading | **DECODED s78**: desired heading = atan2(stickX,stickY)+camera yaw, but the BODY heading +0xC4 is eased toward it and VELOCITY is emitted ALONG +0xC4 (the body, not the raw stick) → motion CURVES into turns | ~~camera-relative stick direction + smooth turn (a re-model)~~ → turn-then-translate-along-body (implement pass in progress) | S | SI→decoded (impl in progress) |
...
| C12 | Player damage | per-frame processor func_0021C440: pending floats +0x224 (health) / +0x22C (infection), type byte D_008102BF, state-2 flinch/death subs, producer-side i-frames (event byte + 0x20E window), corpse hold + fade (FINDINGS "PLAYER DAMAGE & DEATH PIPELINE") | **CLOSED 2026-06-11 s58**: generic tail translated — flinch (decoded clips 0x1E-0x21/0x56-0x57/0x1C7 + voices 0x152/0x153), death sequence (clips 0x2A/0x5C/0x1C4, the 5-sound cue chain, 120-frame hold, 4-speed fade), infection-at-100 latch + 60-cap + drain, kill plane, i-frames. Remaining: typed paths 1..0xB (no native producers), latch ticks, blood-pool/gore effects (no effect system), heartbeat rumble (no FF backend), hazard-room drain (no room attrs) — all flagged. **s77 — the damage INPUT path is now decoded (FINDINGS "ENEMY → PLAYER DAMAGE CONTRACT" §1-2): the player has NO +0x36; the port's `player_hit = 0x4000\|dmg` is a fabrication. Real path = func_001A8BE0 (in func_001AAD00) walks the hazard list → class appliers (1 sphere/3 pad-infection/5 AABB) → +0x224 health / +0x22C infection / +0x0F type, magnitude from per-attacker-kind tables D_0024A7C0/740 (idx = attacker +0x0D; idx5=100=instakill; HARD via D_0081070A). Port REDO: replace the +0x36 player mailbox with the three appliers; breather pad = +5 infection; bug bite = the live latch, NOT the func_001B5360 blood-decal table.** | B | SI (typed paths UT; damage-input REDO flagged s77) |
...
| # | Behavior | Engine truth | Port | Sev | Status |
|---|----------|--------------|------|-----|--------|
| F1 | Character lighting | per-room rigs D_00251C50 (ambient + 2 directionals per room) + always-on camera light (flag +0x2 bit 0x20) + ≤32 dynamic point lights (func_001D89D0 / func_001D7FA0 — FINDINGS s51) | **LARGELY CLOSED (verified s77, FINDINGS "ENEMY → PLAYER DAMAGE CONTRACT + AREA-11 ENEMIES" §6)**: the port shader kernel (em_gfx_metal.m: min(amb + Σ max(dot(N,dir_i),0)·col_i, 255)/128) + char_rig_build (em_game.c: per-room rig, the camera-fill via inverse view rotation, the k=0.1·I/max(d²,1) lamp fold) reproduce the engine and match the .s. Remaining exact-match gaps (none affect the lamp-less snow first level): the func_001D8270 fold type/size gate (port folds for all actors), per-lamp ±1.8° flicker + story-flag lamp gates (office/area-0), and folding the actor RGB mult + self-glow into col/amb BEFORE the 255 clamp (port tints post-modulate) | V | SI (was "the biggest visual gap" in the 2026-06-11 audit — that predated the port's rig work) |
...
| P2 | ~~`TURN_SPEED` 12 rad/s facing seek has no engine citation~~ — DECODED s78 (C3): replaced by the speed×angle-banded turn-rate table (func_00174AC0); implement pass in progress | `em_game.c` (loco turn) |
...
16. **Title screen / front-end menus** — ui_screens cluster (area-name title screens,
    menu flows; game_task states beyond the in-game arm are pending). s77: the boot→
    new-game→first-level FLOW is fully decoded (FINDINGS "NEW-GAME → FIRST-LEVEL PATH":
    the front-end IS the continue machine func_001AC070 with from-death=0; NEW GAME →
    func_001AF2C0 init → AREA 11 snow at (250.8,229.9,209)/yaw 0.611) and the title +
    game-over screen ART is now EXTRACTED (tools/export_screen_modules.py → title.emui /
    gameover.emui, grayscale; color capture-gated). Per the user, the boot stays
    gameplay-first and the title MENU WIRING is deferred — the remaining work is
    generalizing the ported GO/continue skeleton to the from-death=0 TITLE mode + seeding
    the new-game inventory (the f-AF2C0 values) on NEW GAME.

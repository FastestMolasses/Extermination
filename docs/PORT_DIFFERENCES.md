...
| J14 | Crate-hatch BUG (s68, NEW) | the hatchling: 15-node insectoid (legs/antennae/segmented tail), slots 0x0F (chitin) / 0x10 (red flesh; selected by event flag 0x30 = D_00810788==0xFF), clip bank 0x11 (36 containers; init/walk = clip 1), HP func_00128390 15/30 (30/50 hard), +0x36 consumed via func_00128B80 + hurt/death func_00129FC0 (clips 0x1B/0x1D/0x20), spawn-pose code = nest param (func_00129780 cases 0..0xC), brains read player+0xA (light detection) | IN PORT 2026-06-11 as EM_ENEMY_KIND_BUG: variant-A mesh + clip bank (walk = clip 1, flinch 0x1D; death 0x1B is in an unbakeable container — corpse-fade fallback, the code requests the engine id), HP 15, every-tick mailbox flinch/death, victim in all three hitscan filters; the brain is the FLAGGED-MINIMAL walk/approach shape (homing/speed/standoff are port constants, deals no damage). Untranslated: the real brain machines + attack moves, variant B / event flag 0x30, the spawn-pose codes (all hatch floor-posed), player+0xA light detection, difficulty HP column | B | SI (minimal brain; assets in) |

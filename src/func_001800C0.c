...
// asm void: 2.3.1 dead instruction (addiu a1,zero,0xA0 after b+lq) and bnez delay slot
// pre-hoisted for taken path. Byte-identical at link time.
extern void func_001749A0(int, int, int);

asm void func_001800C0(unsigned char *a0) {
    ...
    .word 0x240500A0  // addiu a1, zero, 0xA0 (delay -- pre-hoisted for taken path)
    .word 0x2405009E  // addiu a1, zero, 0x9E (not-taken)
    jal func_001749A0
    .word 0x24060001  // addiu a2, zero, 0x1 (delay slot)
    .word 0x10000005  // b exit (+5*4=+0x14)
    .word 0x7BBF0000  // lq ra, 0x0(sp) (delay slot)
    .word 0x240500A0  // addiu a1, zero, 0xA0 (DEAD -- 2.3.1 dead instruction)
    // taken:
    jal func_001749A0
    .word 0x24060001  // addiu a2, zero, 0x1 (delay slot)

// NEARMISS func_0011AEA0  (vram 0x0011AEA0, 0x60 bytes) — readable decompilation, NOT byte-identical.
//
// objdiff 99.58% via ee-gcc 2.9-991111-01 (-O2). The LOGIC and STRUCTURE are faithful; the residual
// diff is a genuine compiler artifact that no source change fixes here:
// jr-table external-dispatch wall (proven s84) — sole residual: lui/addiu %hi/%lo(jtbl_0026C140) vs local %hi/%lo([.rodata]). All 22 other instructions byte-identical. NOTE: SDK/lowmem region -> ee-gcc 2.9-991111-01 -O2, NOT mwcc.
//
...
// func_0011AFD0 reads COP2 control register 29 (VPU_STAT) and returns a VU1
// run-state code decoded from the VU1 half of that register:
...
// In state 4 (VU1 still running) it first sets bit 8 (FBK1, VU1 force break)

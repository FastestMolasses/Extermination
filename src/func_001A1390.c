// NEARMISS func_001A1390  (vram 0x001A1390, 0x7F0 bytes) — readable decompilation, NOT byte-identical.
//
...
// Same wall class as sibling func_001A1B80 (identical body/shape, only the kind threshold constant differs: 0x51 vs 0x5A): genuine register-coloring + branch-likely-sense scheduling artifact. Full logic/structure recovered (correct 8-saved-GPR frame including the genuinely-needed extra s7 for pass-...
//
...
//   D_00810700/D_00810701, records 0x28 bytes, index = uid) < 0x51.
...
//   at actor +0x54 < 0x51. On hit (checked per-prim, inside the dispatch
...
int func_001A1390(void)
{
    ...
    *(int **)0x700031D0 = &D_700030B0;
    ...
    for (i = 0; i < *(short *)0x7000324C; i++) {
        ...
        if ((*(unsigned char **)((char *)D_0024D7C0[D_00810700] + D_00810701 * 4))[i * 0x28 + 8] >= 0x51) {
            continue;
        }
        {

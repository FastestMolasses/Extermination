// NEARMISS func_001A2AE0  (vram 0x001A2AE0, 0x7DC bytes) — readable decompilation, NOT byte-identical.
//
// objdiff 77.20% via mwcc 2.3.3 (mwcps2-2.3.3-000906) (-O4,p -sdatathreshold 8). The LOGIC and STRUCTURE are faithful; the residual
// diff is a genuine compiler artifact that no source change fixes here:
// Genuine register-coloring + branch-likely-sense scheduling artifact (not idiom-13/inverse-CSE/branch-likely-shape reachable): after full logic/structure recovery (correct frame size, exact f20-f25 clamp coloring, exact switch dispatch order via reversed case labels for the mwcc switch-lowering re...
//
...
// SEMANTICS: segment hit-scan sibling of func_001A32C0/func_001A3980/
// func_001A06A0 (see docs/FINDINGS.md "4. The uid state words at
// 0x70003250"). Clamps the query interval on all three axes (x/y/z,
// D_70003190.98 lo / D_700031A0.A8 hi), then:
//   Pass 1: per-uid table walk at 0x70003250 (count 0x7000324C), gated
//   live/enabled (flags bit31 set, bit30 clear, bit29 gated on
//   D_7000324E) and area-descriptor kind (+8, D_0024D7C0[area][sub] ==
...
//   func_001A44B0 (0x4000/0x8000) / func_001A4650 (0x2000) /
...
//   uid lookup + AABB + prim dispatch, but dispatches 0x4000/0x8000 to
//   func_001A5C30, 0x2000 to func_001A50A0, kind gate direct at actor
//   +0x54 < 0x5A. On hit: re-splats D_70003190[0..2].+0x10 from +0x20 (a
//   3-word vector fixup), stages the hit actor pointer at 0x700031D4,
//   ORs actor kind byte into 0x700030CA, re-clamps all 3 axes.

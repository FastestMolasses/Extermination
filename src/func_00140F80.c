// NEARMISS func_00140F80  (vram 0x00140F80, 0x744 bytes) — readable decompilation, NOT byte-identical.
//
...
// Two residual compiler artifacts, logic/structure fully recovered: (1) clean-store idiom-13 delay-slot fill after bnez guarding func_001B2F70(...)==0 store (mwcc233 fills with lui at,0x7000; target leaves nop, shifting all later branch offsets by +4); (2) FP-register-coloring swap on both scratchp...
//
...
// NEARMISS 98.4% (mwcc233; 991202 = 93.2%) — logic fully recovered.
//
// Actor "turn-toward-target" facing check + band-distance state update, called with
// arg0 = self actor record, arg1 = a per-target-slot state block (arg1+0x48 = a signed
// "phase" float, arg1+0x80 = a bitmask of facing-check results, arg1+0x85/0x86/0x87 =
// mode/flags/timer-ish bytes, arg1+0x4C/0x60 = misc floats).
//
...
// seeds 4 fixed local-space direction vectors (D_700038A0/B0/C0/D0, +-3.0/-+3.0 XZ pairs)
...
// walks 4 direction vectors from a global table D_00245900 (0x10-byte stride, entries 1..4)
// through func_001B32F0(arg0, dir, 45deg) in a loop, same OR-bit-0 result.
//
...
//    0x700038B0..BC); on a hit, resets phase to 0.0f and ORs bit 1 into arg1[0x80] and
//    stops the loop. If bit 1 is still clear and arg0[4]==1 (a "type" byte) and arg1[0x85]
//    is 0 (an idle-cooldown gate), does a one-shot func_001B2F70(arg0+0xB0, scratch) line-
//    of-sight-ish probe: on failure re-seeds the scratch X with (arg0->0xB4 - 13.0f); then
//    if (arg0->0xB4 - scratch.x) is not < 13.0 and phase < -0.3, forces arg0[5]=5/arg0[6]=0
//    (kicks a sub-state machine elsewhere). Regardless, rebuilds a full position+scale quad
//    at the scratchpad from arg0+0xB0..BC (position, +18.0f on the Z/height component,
//    scale 1.0), clears bit 0 of arg1[0x86], and calls func_0019B6C0(scratch, arg0+0xB0,
//    1.0) — a broadphase/room query. On a hit, reads a global cursor object's type byte at
//    *(int*)0x700031D0 + 0x1A; if it's 0x5B (a specific room/trigger type), sets bit 0 of
//    arg1[0x86], copies the global float at 0x700031B4 into arg1[0x60], and if arg1[0x87]==0
//    and arg1[0x4C]!=0 picks a sound/vfx variant `((func_00122BB8(type)>>9)&7)+7` into
//    arg1[0x87] and fires func_001EFD90(0x8000001D, pos, arg0+0xC0) (a positioned event).
//  f > 0: rebuilds a position+scale quad from arg0+0xB0..BC (position, +20.002f on Z, band
//    15.0f at 0x700038B0..BC, scale 1.0) and calls func_0019AB20(arg0, pos, band, 0x80000007);
//    on a hit, resets phase to 0.0f and ORs bit 2 into arg1[0x80].
//  f == 0: no-op (neither branch taken).
...
extern int func_0019B6C0(void *a0, void *a1, float f12);
    ...
    if (f < 0.0f) {
        ...
        *(int *)0x700038BC = 0x3F800000;
        ...
        if (!(*(char *)(arg1 + 0x80) & 2) && *(unsigned char *)(arg0 + 4) == 1 && *(char *)(arg1 + 0x85) == 0) {
            ...
            if (!(*(float *)(arg0 + 0xB4) - *(float *)0x700038A0 < 13.0f) && *(float *)(arg1 + 0x48) < -0.3f) {
                *(char *)(arg0 + 5) = 5;
                *(char *)(arg0 + 6) = 0;
            }
        }
        ...
        *(float *)0x700038A4 = 18.0f + *(float *)0x700038A4;
        *(char *)(arg1 + 0x86) = *(char *)(arg1 + 0x86) & ~1;
        if (func_0019B6C0(D_700038A0, arg0 + 0xB0, 1.0f) != 0) {
            unsigned char t;
            t = *(unsigned char *)(*(char **)0x700031D0 + 0x1A);
            if (t == 0x5B) {
                *(char *)(arg1 + 0x86) = *(char *)(arg1 + 0x86) | 1;
                *(float *)(arg1 + 0x60) = *(float *)0x700031B4;
                if (*(char *)(arg1 + 0x87) == 0 && *(float *)(arg1 + 0x4C) != 0.0f) {

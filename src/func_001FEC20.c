...
//
// State-machine step (state 0xD). Reads the shared mode word D_00275C5C:
//   - state 0: arm this step (set state to 0xD), kick off func_00114AC0(); if
//     it reports busy (nonzero) raise the busy flag D_00275C58, then return -1.
//   - state 0xD: when the result word D_00275C64 is non-negative, clear the
//     state and return that result; otherwise raise the busy flag, return -1.

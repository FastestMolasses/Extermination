...
// mwcc 2.3.3 reproduces the CW exit/delay-slot lowering (a bne to the shared END with the
// return-0 register-clear idiom filled into the delay slot) and the
// v0 = a1 - 0x34 form that the pinned 991202 build does NOT (991202 reloads
// via a byte mask of a1, scoring 83.2%). The nested single-exit shape

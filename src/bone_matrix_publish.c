...
// 0x31 or 0x34, calls anim_clip_arbiter(obj, id, 0.0f, (float)objp[+0x276]); else
// passes 1.0f. Then anim_eval_skeleton(obj) (anim_eval_skeleton). Finally, for each
// of objp[0xC] bones, copies the qword matrix at (*node)+0x90 into dst[i] via
// copy_qw4, where node = *(int *)((char *)D_00275B40 + i*4) — D_00275B40
...
extern int anim_clip_arbiter(unsigned char *obj, int id, float a, float b);
extern void anim_eval_skeleton(unsigned char *obj);
extern void copy_qw4(Mat4 *dst, Mat4 *src);
...
void bone_matrix_publish(unsigned char *objp, int id, Mat4 *dstp)
{
  ...
  if (type == 0x31 || type == 0x34) {
    ...
    anim_clip_arbiter(obj, id, 0.0f, 1.0f);
  }
  anim_eval_skeleton(obj);

  for (i = 0, off = 0; i < obj[0xC]; i++) {
    copy_qw4(dst, (Mat4 *)(*(int *)((char *)D_00275B40 + off) + 0x90));
    off += 4;
    dst++;
  }
}

...
// usual -O2 ee-gcc colours the compare constant 3 into $a0; the original
// reuses the now-dead %hi address register ($v0) for it. -Os
// alone colours the load into $v0 instead, and -O2 -fno-schedule-insns colours
// the load into $a0 — only the pair reproduces the original's assignment:
// address high half in $v0, loaded word in $v1, then the constant 3 back in $v0.

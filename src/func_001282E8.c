...
// SDK/eegcc-convention func (64-bit return-address save, 16-byte stride, struct@sp+0x0 / float@sp+0x10).
// Spills the float arg, calls func_001278C0(&f, &s) to fill a 2-word out-struct, then
// sets s.b = (s.b < 1)  (an unsigned set-less-than against 1, i.e. s.b becomes 1 iff it was 0),
// and calls func_001277B0(&s). -fno-schedule-insns keeps the ra-save in prologue order.
// (Pure mwcc maxes at ~60% -- saves ra as a 128-bit quadword + reorders the prologue; ee-gcc 2.9 is byte-exact.)

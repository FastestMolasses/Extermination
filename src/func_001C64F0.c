// NEARMISS func_001C64F0  (vram 0x001C64F0, 0x2F0 bytes) — readable decompilation, NOT byte-identical.
//
...
// Best build is mwcc991202 (93.16%) vs mwcc233 (92.39%). Body/control-flow fully recovered (per-frame anim clip-time advancer + event-table scan + rotation-keyframe re-resolve dispatch, matches docs/FINDINGS.md's func_001C64F0 description). Residuals are pure compiler-scheduling artifacts, not logi...
//
...
short func_001C64F0(char *arg0, float dt) {
    float t;
    float step;
    short flags;
    char *hdr;

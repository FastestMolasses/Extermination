...
6/13) RMW fills delay slot (p->f|=1 -> byte load, OR-immediate, byte store); CLEAN const store (p->f=1 -> load-immediate then byte store) ALWAYS fills slot = genuine wall.
...
12) FP-param pairing POSITIONAL: N float params across a call save TOP-DOWN, param k -> f(20+(N-1)-k). Inline 0.0f literal when the float compare's operands are f0 then f21.
13b) dead-const re-materialization: if/else-if arms all passing the SAME large two-half (upper/lower immediate) const to a call, or two stores of the same nonzero const straddling a slot-less branch.

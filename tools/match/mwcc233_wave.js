...
FP-ARG-ORDER IDIOM (try this BEFORE parking an fp-move-order near-miss): when a call like anim_clip_init(p, idx, A, 0.0f) has the target emitting the two arg moves as 'mtc1 zero,f13' (the trailing 0.0f) BEFORE 'mtc1 v0,f12' (the A value), but mwcc emits them swapped (f12 then f13), write the trailing 0.0f arg as an assign-in-arg temp to flip mwcc's emit order: 'float z; ... anim_clip_init(p, idx, A, (z = 0.0f));' or 'anim_clip_init(p, idx, A, z=0.0f)'. (This is the permuter-discovered fix for the recurring f13-before-f12 wall — func_00188250/func_00137C80/func_0013B5B0 class.)
INVERSE-CSE IDIOM (idiom-19, try BEFORE parking a frame-size/extra-save near-miss): if the target recomputes a repeated subexpression at each use (two identical subu/addu emissions) but mwcc CSEs it into a callee-saved reg — adding a save and growing the frame (the 0x90->0xa0 frame-size tell) — do NOT share a temp: INLINE the expression literally at every use site INCLUDING inside the branch condition that guards it, so mwcc recomputes it per-path and keeps the frame small. Pair with a no-prototype call decl ('int f();') when arms pass a varying arg count / carry a leftover param. (Cracked func_00203F40: inlined '(a1-t1)' into its if-condition + both uses.)
BRANCH-LIKELY IDIOM (idiom-20 — mwcc DOES emit beql/bnel/beqzl; do NOT park "mwcc can't do branch-likely", that's FALSE, 73 matched mwcc funcs have it, e.g. src/func_0014E4F0.c): mwcc emits a LIKELY branch when it can fill its delay slot with a speculatable pure-ALU op from the TAKEN path. For a switch state-machine: 'switch(st)' on a LOCAL 'st = *(unsigned char*)(e+6)', case labels ASCENDING (mwcc reverses to descending dispatch), and write the state advance as '*(unsigned char*)(e+6) = st + 1;' USING the switch local 'st' (NOT (*(e+6))++ and NOT a reload) -> mwcc drops 'addiu v0,st,1' into the dispatch branch slot via beql/beqzl. Early-return guard 'if(cond){body} return X;' -> '<inv-cond>l epilogue' + X-setup in slot. If the sole residual is "target beql/beqzl where mine has plain beq/bne", reshape so the target's slot-filler op is the first statement of the taken path.
...
NEARMISS HARVEST (IMPORTANT — readable near-miss C is now committable as port ground truth): if you canNOT reach 100.0 but you FULLY RECOVERED THE LOGIC and the sole residual is a genuine compiler artifact (register coloring / instruction scheduling / param-save order / FP-coloring / branch-likely-sense / frame-stride / sibling-call — body/structure correct, only the artifact differs), STILL return your best READABLE c_source (the plain-C version, NOT permuter-mangled) with matched=false, the real measured pct, and a precise wall. The orchestrator commits these as documented // NEARMISS files (boot ELF stays byte-identical via the splat .s; matched_code unaffected). Leave c_source EMPTY only for: jr-table/VU0 (no valid C), or a decode you are NOT confident is logically correct. Better to return faithful readable C at 70% than nothing.

Report per func: func, pct_991202, pct_233, matched, compiler, c_source (full committed-ready file if matched=true at 100.0, OR best readable near-miss file if body-correct per NEARMISS HARVEST), wall (precise reason if not 100.0).`

...
- idiom-14 (clean-store delay-slot NOP — the "genuine cleanstore" parks are USUALLY this): a cond-branch (beqz/bnez/bc1f/bc1t/beq/bne) whose delay slot is a NOP in the target. mwcc leaves the slot NOP for FREE iff the FIRST EMITTED instr of the success path is a STORE/LOAD/CALL (never speculated). So: write the success block's FIRST statement as a store of an ALREADY-LIVE value — \`p->f = 0\`, \`p->f = <live param/saved reg>\`, or \`p->f = <precomputed local>\` (NO materialization needed) — and the nop appears at zero cost. ONLY a store of a value that must be MATERIALIZED (\`p->f = 3\` -> the load-immediate fills the slot) is a GENUINE wall. If the success block really begins with a load/store/call, make THAT the first if-body statement. Multi-statement blocks are fine; const stores emit after.
...
- FP-ARG-ORDER idiom (the fparg parks): a call like f(p, idx, A, 0.0f) where the target moves the trailing 0.0f into f13 BEFORE it moves A into f12, but mwcc fills f12 then f13 — flip mwcc's order by writing the trailing arg as an assign-in-arg temp: \`float z; ... f(p, idx, A, (z = 0.0f));\`. Also idiom-12: a single \`float\` param live across a call colors by decl order; an inline \`0.0f\` literal in a compare gives the float compare operands in f0-then-f21 order (a \`float zero\` local reverses it).

...
The decoded output will contain a real \`switch (x) { case 0: ... }\`. If m2c instead leaves a raw \`*((idx*4)+&jtbl_XXXX)\` expression, recover the switch BY HAND from the target .s: the dispatch loads the index byte, bounds-checks it against N, loads the table entry and jumps through it; read the N jtbl entries (jtbl_prep printed the .jtbl.s with the \`.word .L<addr>\` targets in case order) and write \`switch(idx){case 0..N-1}\`.
...
C) idiom-14 STORE-OF-LIVE-VALUE de-wall: a clean store of an already-live value (\`p->f=0\`, \`p->f=<param>\`, \`p->f=<live local>\`) leaves a cond-branch delay slot a NOP at zero cost (the store follows the nop). Only a MATERIALIZED-const store (\`p->f=3\`: load-immediate then byte store) fills the slot = genuine wall.
...
6/13) RMW fills delay slot only for read-modify-write (\`p->f|=1\`: byte load, OR-immediate, byte store); clean const store always fills slot = wall.
...
12) FP-param pairing POSITIONAL: N float params live across a call save TOP-DOWN, param k -> f(20+(N-1)-k). Inline \`0.0f\` literal when the float compare's operands are f0 then f21.
13b) dead-const re-materialization: force a dead-dup via if/else-if arms all passing the SAME large two-half (upper/lower immediate) const to a call, or two stores of the same nonzero const straddling a branch with no store for the slot.
...
18) s64-param store: declare a 64-bit-stored param \`long long\` for a 64-bit store of its saved register (no sign-extension shift pair).

...
PER-FUNCTION LOOP:
1. Read build/asm/matchings/main/code/<F>.s — decode as normal gcc-2.x MIPS (sd/ld 64-bit saves, daddu moves, calls with an unfilled nop delay slot are NORMAL). Many SDK funcs are small syscall stubs / thin wrappers / list ops / bignum-math — very matchable. The existing src/<F>.c (stub) may document semantics.
...
4. Iterate to 100.0 (cap ~6 attempts). Most SDK leaves match at plain -O2. If it resists, try -O1 or -O0 in the CFLAGS line. ee-gcc notes: tail-call wrappers (a plain jump to the callee) may need a non-tail-call rewrite (gcc 2.9 has no -fno-optimize-sibling-calls); 64-bit long arithmetic uses the 32-bit shift pair for sign extension; globals via a %hi/%lo address pair — plain externs first, rarely gp-rel.
...
EEGCC IDIOM-29 — POINTER-LOCAL %hi (this DE-WALLS the recorded "%hi-caching-in-callee-reg" no-lever wall; try it FIRST on any coloring/%hi residual): writing 'D_xxx = f(1);' makes ee-gcc emit the store's %hi upper-half load AFTER the call in a CALLER-saved register. Assigning the address to a local first — 'int *p; p = &D_xxx; *p = f(1);' — forces the %hi BEFORE the call into a CALLEE-saved register, which is what the target does. Worth +35% on func_00106E30 (38.6% -> 100.0). Globals whose %hi the target hoists to the very TOP of the function need their OWN dedicated pointer locals declared at function entry (pa/pb), not the reused 'p'.
EEGCC IDIOM-30 — ONE TEMP PER CALL RESULT: a single reused 'int t' for several call results becomes ONE pseudo with a function-long live range that cannot coalesce with $v0, so ee-gcc emits spurious 64-bit copies of v0 into argument registers. Use separate t1..tN, one per call. Worth +17% on the same function.
EEGCC IDIOM-31 — PRE-READ FOR THE DELAY SLOT: reading a struct field into a local BEFORE a dependent global store lets the delay-slot pass fill the following 'bnez' with that store instead of emitting an annulled 'beqzl'.
EEGCC ANTI-IDIOM — do NOT mark hardware-register accesses (e.g. 0x10002010) 'volatile' unless the diff demands it: volatile BLOCKS the final delay-slot fill and cost the last 2% on func_00106E30.

FAST-PARK (≤2 tries — these are CONFIRMED uncrackable s84, do NOT burn the full budget):
EEGCC IDIOM-1 — FORWARD BRANCH-LIKELY IS MATCHABLE (s85; the old 'wall' was WRONG, do NOT park it): if expected has bgezl/blezl/bgtzl/bltzl/beql/bnel/beqzl/bnezl on a FORWARD (non-loop) conditional and yours emits the same op without the trailing 'l', that is NOT a wall. gcc's fill_eager_delay_slots emits the annulled form when the BRANCH-TARGET path's FIRST instruction is a cheap speculatable op — in practice a lw of a global through an ALREADY-LIVE base register (no lui setup). PROOF: func_00113F68 matches 100.0 with a forward branch-likely on the sign of the call result whose annul slot holds the word load of D_00241CF8 through an already-live base register, from: 'if (func_0010E8A8(...) < 0) { CreateSema(D_00241D08); return 0; } if (D_00241CF8 > 0) func_00122B58(...);' Read the .s to see WHICH op is in the annul slot, then make that op the first statement of the taken path. A taken path leading with a lui re-materialization will NOT annul — that single failure was over-generalized into a bogus wall. Spend real attempts here.

...
GPREL_FORCE_ASM = {
    # STALE-ENTRY PRUNE (m3-matching lane, 2026-09-23). 52 entries were removed
    # from this list and 57 from SIZE_DRIFT_FORCE_ASM: every one had ordinary-C
    # source at objdiff 100%, and with the entry removed fill_unmatched copied
    # the compiled object (audit_link_provenance: compiled_object_ordinary_c,
    # prepared text + relocations equal to the filler) and link.py still
    # verified the boot ELF byte-identical. strip_sections now applies
    # address-named GPREL16 fixups, so the old overflow no longer occurs.
    # Six gprel entries whose compiled object carries .rodata stay listed (the
    # local-data guard would force them to .s anyway, so their compiled linkage
    # is unproven): func_00159210, func_00159620, func_001BC960, func_001BDFC0,
    # func_001BF6B0, func_00207350.
    # Second round (same lane): 36 more entries (2 here, 34 in SIZE_DRIFT)
    # whose ordinary C now reaches objdiff 100% -- 19 SDK-region units by
    # compiling them with ee-gcc (// COMPILER: eegcc), the rest with mwcc 2.3.3
    # and small source fixes -- were removed with the same proof: a full
    # build.py build + verify_all (boot ELF byte-identical, 19/19 overlays)
    # and audit_link_provenance showing compiled_object_ordinary_c for each.
    # What remains (audit_link_provenance, 2026-09-23): here, 50 NEARMISS
    # units (linked from the .s by design), the six .rodata-carrying 100%
    # objects named above, and 6 ordinary-C units below objdiff 100%. In
    # SIZE_DRIFT_FORCE_ASM, 51 ordinary-C units below 100%, 1 NEARMISS, 2
    # inline-assembly units below 100%, and 95 INLINE-ASSEMBLY units that ARE
    # objdiff 100% -- a class this round did not examine (it only proved
    # ordinary-C entries); whether they still need listing is open.
    # Seven dead names were also dropped (3 here: func_001C0004,
    # func_001D0F20, func_002293B0; 4 in SIZE_DRIFT: func_0010AD48/58/68/78):
    # none has a src/*.c, a splat .s or a link row -- they were merged away by
